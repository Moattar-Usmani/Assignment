...
class Guessme:
    ...
    def structure(self, master):
        ...
        self.inst_lbl = Label(master, text="Welcome to Guessme Game!")
        ...
        self.inst_lb3.grid(row=1, column=0, columnspan=2, sticky=W)
        # Creating label to display current Guessed Status of Word

        self.curr_char1 = Label(master, text=p)
        self.curr_char1.grid(row=4, column=1)
        self.curr_char = Label(master, text="Current Status:")
        self.curr_char.grid(row=4, column=0)
        # Create a Guessme's Background
    ...
    def submit(self, *args):
        ...
        # Creating Guess me character’s parts orderwise if wrong character is Guessed
        if (self.count == 1):
        ...
        # Condition Of player Loose
            ...
            messagebox.showinfo("YOU LOSE!", "You lost please try again!")
            self.rr.destroy()


while True:
    root = Tk()
    root.title("Guessme Game")
    ...
    root.withdraw()
    # the input dialog
    USER_INP = simpledialog.askstring(title="Guessme Game",
